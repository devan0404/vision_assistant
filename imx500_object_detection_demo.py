...
last_detections = []
last_output_time = 0
OUTPUT_INTERVAL = 2.0  # Increased to reduce SSH traffic
MIN_CONFIDENCE = 0.70  # Increased to reduce false positives
MAX_DETECTIONS = 3     # Reduced to minimize data transfer
FRAME_SKIP = 3        # Increased to reduce processing load
MIN_CONFIDENCE_CHANGE = 0.05  # Only output if confidence changes significantly
...
if __name__ == "__main__":
    try:
        ...
        config = picam2.create_preview_configuration(
            controls={"FrameRate": 10},  # Reduced frame rate
            buffer_count=4  # Reduced buffer count
        )

        imx500.show_network_fw_progress_bar()
        picam2.start(config, show_preview=False)  # Disabled preview for SSH
        ...
        while True:
            ...
            if now - last_output_time >= OUTPUT_INTERVAL:
                try:
                    ...
                    # Only output if there are new detections or significant changes
