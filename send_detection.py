...
def format_detection(detection_data):
    """Format the detection data into the expected structure."""
    formatted_detections = []
    for det in detection_data:
        if isinstance(det, dict) and 'label' in det and 'confidence' in det:
            formatted_det = {
                'label': det['label'],
            ...
            try:
                if line.strip().startswith("[") and line.strip().endswith("]"):
                    ...
                    if detection_data:  # Only update if we got actual detections
                        formatted_data = format_detection(detection_data)
                        if formatted_data:  # Only update if we have valid detections
                            latest_detection = formatted_data
                            logger.info(f"Received detection: {formatted_data}")
